...
- **Token choice is semantic, not decorative.**
  - **Bechdel** (`movies/sections` inline in `+page.svelte`, `.criterion` / `.criterion-marker`) is a **cumulative ladder**: 3 criteria as a numbered list with a vertical connector line (`::before`, tinted `color-mix(--brand 45%, --border)` once met). Met markers use the `--brand` tint — pass/fail itself lives only in the section-header status pill, so no separate banner. Avoid `--success`/`--danger` here; it clashed with the page's brand/neutral language.
